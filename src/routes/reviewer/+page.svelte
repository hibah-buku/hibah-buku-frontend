<script>
		...
		if (token && userStr) {
			try {
				...
				if (parsedUser.role !== 'reviewer') {
					...
					token = token;
					setAuthToken(token);
					reviewerId = String(user.reviewer_id);
					fetchAssignments();
				}
			} catch (e) {
				goto('/');
	...
	async function fetchAssignments() {
		...
		loadError = '';
		try {
			const res = await api.get('/reviewers/' + encodeURIComponent(reviewerId) + '/assignments');
		...
		if (a.status === 'completed') {
			try {
				const [detailRes, resultsRes] = await Promise.all([
					api.get('/assignments/' + a.id),
					api.get('/assignments/' + a.id + '/results')
				]);
	...
	async function previewBook(a) {
		try {
			const res = await api.get('/assignments/' + a.id + '/preview');
			const data = res.data?.data || {};
			const targetUrl = data.url || data.preview_url || API_BASE_URL + '/assignments/' + a.id + '/preview';
	...
	async function submitReview() {
		...
		try {
			...
			const res = await api.post('/assignments/' + activeAssignment.id + '/reviews', payload);
...
<div class="min-h-screen bg-gray-50">
	<!-- HEADER -->
	<div class="bg-gradient-to-r from-indigo-700 to-blue-600 px-4 py-8 text-white shadow">
		<div class="mx-auto flex max-w-6xl items-center justify-between">
			<div>
				<h1 class="text-3xl font-bold tracking-tight">Dashboard Reviewer</h1>
				<p class="mt-1 text-sm text-blue-100/90">Sistem Penilaian Kelayakan Buku Referensi dan Ajar</p>
			</div>
			<div>
				{#if user}
					<div class="flex items-center gap-4 rounded-full border border-white/20 bg-white/10 p-1 pr-4 shadow-inner backdrop-blur-sm">
						<div class="flex items-center gap-2 pl-3">
							<div class="flex h-8 w-8 items-center justify-center rounded-full bg-white text-sm font-bold text-indigo-700 shadow-sm">
								{user?.name?.charAt(0).toUpperCase() || 'U'}
							</div>
							<div class="flex flex-col">
								<span class="text-xs font-bold tracking-wide text-white">{user?.name}</span>
								<span class="text-[10px] font-medium tracking-wider text-blue-200 uppercase">{user?.role}</span>
							</div>
						</div>
						<button onclick={logout} class="ml-2 rounded-full bg-red-600 px-4 py-1.5 text-xs font-bold text-white shadow-md transition-all hover:bg-red-500 hover:shadow-lg active:scale-95">
							Logout
						</button>
					</div>
				{/if}
